...
public final class ImageByteMap {
    ...
    boolean addRegion(Region region) {
    	...
    	if ((region.getStartAddress() < 0) || (region.getLength() == 0)) {
    		Msg.debug(this, "addRegion: invalid range");
    		return false;
    	}
    	
    	Msg.debug(this, "Adding new region:");
		Msg.debug(this, "From " + String.format("0x%08X", region.getStartAddress()) 
    	...
    	Msg.debug(this, "Calculated insert position: " + insertPosition);
        ...
        if (insertPosition > 0) {
        	...
        	if ((belowRange.getStartAddress()+belowRange.getLength()) > region.getStartAddress()) {
        		Msg.debug(this, "addRegion: overlapping regions not permitted");
        		return false;
        	}
		...
		Msg.debug(this, "addRegion: size " + size);
		Msg.debug(this, "addRegion: minAddress " + this.minAddress);
		Msg.debug(this, "addRegion: maxAddress " + this.maxAddress);
    ...
    void printImageByteMap() {
    	...
    	//while (listIter.hasNext()) {
    	//	Region thisRange = listIter.next();
    	//	System.out.println("MemRange " + regionList.indexOf(thisRange) + " addr " + thisRange.getStartAddress());
    	//}
    	for (Region range: this.regionList) {
    		long startAddr = range.getStartAddress();
    		int length = range.getLength();
    		long endAddr = startAddr + length - 1;
    		Msg.debug(this, "From " + String.format("0x%08X", startAddr) + " To " + String.format("0x%08X", endAddr) +
    		"     size: " + String.format("%d", length) + " (" + String.format("0x%x)", length));
    	}
    }
	...
	byte[] getBytes(long startAddress, int size) {
		...
		System.arraycopy(region.getBytes(), (int) (startAddress - region.getStartAddress()), replacement, 0, size);
		return replacement;
	}
