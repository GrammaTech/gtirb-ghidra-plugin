...
public class gtirbLoader extends AbstractLibrarySupportLoader {
	...
	public final static String GTIRB_NAME = "GrammaTech's IR for Binaries (GTIRB)";
    ...
    private boolean initializeFunctionMap (Module m) {
        ...
        for (Map.Entry<UUID,ArrayList<UUID>> entry : functionEntries.entrySet()) {
            ...
            gtirbFunction function = new gtirbFunction(functionName, functionAddress, functionSize, feUuid);
	...
	public boolean addFunction(gtirbFunction function, Program program) {
		...
		try {
			this.listing.createFunction(function.getName(), entryAddress, body, SourceType.IMPORTED);
		}
		catch (Exception e) {
	...
	protected void load(ByteProvider provider, LoadSpec loadSpec, List<Option> options,
        ...
        // Language specification (ISA/Endian/FileFormat) is chosen before choosing which loader to use. 
        // The process for selecting language spec involves .opinion files and I haven't figured it out yet.
        // Currently I am letting Ghidra autoselect the language spec, then forcing the choice of loader
        // based on the file extension of ".gtirb". See this.findSupportedLoadSpecs().
        //
        // I may be able to resolve this by add/modifying opinion files. If not, I have to do one of these things:
        // - Find a way to change language specification if the module does not match what got me here, or
        ...
        if (module.getFileFormat() == Module.GTIRB_FILE_FORMAT_ELF) {
        	...
        	Msg.debug(this, "File Format is not ELF (ID = " + module.getFileFormat() + "), so far only ELF is supported");
        }
        if (module.getISA() == Module.GTIRB_ISA_X64) {
        	Msg.debug(this, "ISA is X64.");
        } else {
        	Msg.debug(this, "ISA is not X64 (ID = " + module.getISA() + "), so far only X86 is supported");
        }
        ...
        try
        {
        	storageUndefined = symbolTable.createNameSpace(null, "Storage_Undefined", SourceType.IMPORTED);
        	// TODO: This namespace is actually not needed:
        	// Normal means global, should use null instead
        	storageNormal = symbolTable.createNameSpace(null, "Storage_Normal", SourceType.IMPORTED);
        	// Static means in the module namespace.
        	if (module.getName().isEmpty()) {
        		storageStatic = symbolTable.createNameSpace(null, "MODULE_NAME", SourceType.IMPORTED);
        	} else {
        		storageStatic = symbolTable.createNameSpace(null, module.getName(), SourceType.IMPORTED);
        	}
        ...
        //
        // The other option to look at is like that of ELF (see markupSymbolTable in elfProgramBuilder)
        // in which the symbol table is loaded at a certain address, the block is cast as "Data" and iterated through,
        // and the symbol address is the address in that table, references should then be created for each data
        // or code referent.
        // OR, maybe I should actually be looking at the .symtab section? At least there all the symbols
        ...
        for (Symbol symbol: symbols) {
        	...
        	if (referent == null) {
        		...
        		if (symbol.getStorageKind() == Symbol.GTIRB_STORAGE_LOCAL) {
	        		...
	        		if (function == null) {
	        			Msg.error(this, "No function found for address " + String.format("0x%X", rawAddress));
	        			continue;
	        		}
	        		...
	        		if (localNamespace == null) {
	        	        try {
	        	        	localNamespace = symbolTable.createNameSpace(null, function.getName(), SourceType.IMPORTED);
	        	        }
	        	        catch (Exception e) {
	        	        	Msg.error(this, "Error creating namespace for function" + function.getName());
	        	        	continue;
	        	        }
