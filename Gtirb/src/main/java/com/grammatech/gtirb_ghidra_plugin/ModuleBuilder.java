 ...
 * state of Ghidra's ${@link Program}.
 */
public class ModuleBuilder {
    private final Program program;
    private final boolean enableDebugMessages = false;

    public ModuleBuilder(Program program) { this.program = program; }

    /**
     * Get a block that starts at the specified address, splitting to make a
     * new block if necessary.
     */
    public static UUID splitBlocksAtOffset(Module module, long offset,
        ...
        for (Section curSection : module.getSections()) {
            for (ByteInterval curInterval : curSection.getByteIntervals()) {
                OptionalLong intervalAddress = curInterval.getAddress();
                if (intervalAddress.isEmpty())
                    continue;

                newBlockOffset = offset - intervalAddress.getAsLong();
                if (newBlockOffset >= 0 &&
                    newBlockOffset < curInterval.getSize()) {
                    section = curSection;
        ...
        boolean isExecutable =
            section.getSectionFlags().contains(Section.SectionFlag.Executable);
        if (forceExec && !isExecutable) {
            Msg.info(module,
                     "Skipping non-executable CodeBlock: " + section.getName() +
        ...
        for (ByteBlock block : byteInterval.getBlockList()) {
            ...
            // Check whether the new block should fill a gap between existing
            // blocks
            if (newBlockOffset < oldBlockOffset) {
                break;
            }
        ...
        // Split this block to make a new block that starts at the desired
        // address
        ByteBlock newBlock;
        long newSize;
        long decodeMode = 0;
        if (oldBlock != null) {
            ...
            // Adding a new block in empty space. oldBlockOffset here refers to
            // the *next* block.
    ...
    /**
     * Create a new Gtirb section based on the contents of a Ghidra
     * memoryBlock.
     */
    private Section exportSection(Section section, MemoryBlock memoryBlock,
                                  Module module) throws ExporterException {
        if (section != null) {
            // TODO copy UUIDs from source Gtirb, but export everything else
            // from memoryBlock
            return section;
        }
        ...
        /* Gtirb also specifies a ThreadLocal section flag, but Ghidra doesn't
           have one here. Conversely, we aren't currently using Ghidra's
           isMapped, isOverlay, or isVolatile flags.
         */
        ...
        if (memoryBlock.isInitialized()) {
            try {
                ...
                throw new ExporterException("Error reading section bytes: " +
                                            memoryBlock.getName());
            }
        }
    ...
    private Comments exportComments(Module module) {
        ...
        Map<Long, ByteBlock> addrToBlockMap =
            GtirbUtil.getAddrToBlockMap(module);
        ...
        while (addressIterator.hasNext()) {
            ...
            if (commentString != null) {
                ...
                OptionalLong blockAddress = block.getAddress();
                if (blockAddress.isEmpty()) {
                    continue;
                }
                long offset = longAddress - blockAddress.getAsLong();
     ...
     * @return An ImageBase-relative address, or 0 if the symbol has no referent
     * in code or data.
     * */
    ...
    /**
     * Creates a map of all symbols which existed in an imported GTIRB and have
     * since been renamed.
     */
    private HashMap<String, String>
    findRenamedSymbols(ArrayList<Long> sharedAddresses,
    ...
    /**
     * Export the symbols that originally existed in the imported GTIRB,
     * preserving the UUIDs even if the symbol was renamed by the Ghidra user.
     * */
    // Strategy:
    ...
    private void exportRenamedSymbols(Module module) {
        ...
        for (Symbol symbol : module.getSymbols()) {
            ...
            OptionalLong optSymbolAddress = getSymbolAddress(symbol);
            if (optSymbolAddress.isEmpty()) {
                continue;
            }
            long symbolAddress = optSymbolAddress.getAsLong() + imageOffset;
            if (symbolIndex.containsKey(symbolAddress)) {
    ...
    private void exportSymbols(Module module) {
        ...
        for (ghidra.program.model.symbol.Symbol sym :
            ...
            UUID refUuid = null;
            if (!sym.isExternal()) {
                refUuid = splitBlocksAtOffset(
                    module, sym.getAddress().subtract(program.getImageBase()),
                    false, 0);
            }
            if (refUuid != null)
    ...
    private CodeBlock getEntryPoint(Module module) {
        ...
        /* Ghidra standard loaders create an "entry" symbol for the entry point
         * it gets from the format header. This isn't perfect; it doesn't seem
         * to work if the imported binary already defines an "entry" symbol, but
         * I can't find any other way in the Ghidra API to get the entry point
         * corresponding to ELF's e_entry, PE's AddressOfEntryPoint, etc. This
         * "entry" symbol seems to overwrite any existing entry point symbol,
         * such as the
         * "_start" symbol that otherwise should exist for glibc programs.
         */
        for (ghidra.program.model.symbol.Symbol entrySym :
             symbolTable.getGlobalSymbols("entry")) {
            ...
            for (Symbol symbol : module.getSymbols()) {
                if (symbol.getName().equals(entrySym.getName())) {
                    Node entryNode = symbol.getReferent();
    ...
    public Module exportModule(com.grammatech.gtirb.Module module)
        throws ExporterException {
        ProgramModule programModule =
            program.getListing().getDefaultRootModule();
        ...
        if (module == null) {
            ...
            if (isa == Module.ISA.ISA_Undefined) {
                throw new ExporterException("Unsupported processor: " +
                                            lang.getLanguageID());
            }
            if (format == Module.FileFormat.Format_Undefined) {
                /* It may be useful in the future to let this plugin export
                   anything that Ghidra was able to import. If we choose to
                   support that, we probably should add the
                   LanguageID string to an AuxData schema for Ghidra-specific
                   information. */
                throw new ExporterException("Unsupported source format: " +
                                            fileFormatDesc);
            }

            module = new Module(program.getExecutablePath(),
                                program.getImageBase().getOffset(), 0, format,
                                isa, program.getName());
            module.setByteOrder(program.getLanguage().isBigEndian()
                                    ? Module.ByteOrder.BigEndian
                                    : Module.ByteOrder.LittleEndian);
        }
        ...
        if (!isNewModule) {
            for (Section section : module.getSections()) {
                MemoryBlock memoryBlock =
                    program.getMemory().getBlock(section.getName());
                sectionList.add(exportSection(section, memoryBlock, module));
            }
        } else {
            for (Group group : programModule.getChildren()) {
                MemoryBlock memoryBlock =
                    program.getMemory().getBlock(group.getName());
                if (memoryBlock.getName().equals(
                        MemoryBlock.EXTERNAL_BLOCK_NAME)) {
                    continue;
                }
