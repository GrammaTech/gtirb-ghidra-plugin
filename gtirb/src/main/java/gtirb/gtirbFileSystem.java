...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = gtirbFileSystem.MyFileSystemFactory.class)
public class gtirbFileSystem implements GFileSystem {
	 ...
	 */
	public gtirbFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + gtirbFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryFull<gtirbFileSystem>, GFileSystemProbeFull {

		@Override
		public gtirbFileSystem create(FSRL containerFSRL, FSRLRoot targetFSRL,
				ByteProvider byteProvider, File containerFile, FileSystemService fsService,
				TaskMonitor monitor) throws IOException, CancelledException {

			gtirbFileSystem fs = new gtirbFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
