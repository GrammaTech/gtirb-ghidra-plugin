...
public class CFGBuilder {
    ...
    //
    // The address to block map is needed because edges are stored as UUID-UUID
    // pairs, (each UUID identifying a code block), while Ghidra considers an
    // edge to be a pair of addresses. Exporting CFG requires translating Ghidra
    // edges to GTIRB edges, and the most efficient way to get a block UUID from
    // an address is to create a map of addresses to blocks before starting the
    // export.
    ...
    private UUID getBlockAtAddress(Module module, long address, long size) {
        ...
        if (uuid == null) {
            uuid =
                ModuleBuilder.splitBlocksAtOffset(module, address, true, size);
            if (uuid != null) {
    ...
    /**
     * Creates a GTIRB CFG object representing the edges that Ghidra knows
     * about.
     * @param cfg     An existing CFG from the imported GTIRB, or null if one
     * does not exist.
     * @param module  An exported module that contains all the blocks that edges
     * should be generated for.
     * @return
     */
    public CFG exportCFG(CFG cfg, Module module) throws ExporterException {
        // Edges that existed in the original GTIRB, represented by the source
        // and target UUIDs.
        ...
        // Copy all edges from the original GTIRB to the new CFG. Skip if we
        // have none.
        if (cfg != null) {
        ...
        if (!initAddressToBlockMap(module)) {
            throw new ExporterException(
                "Export CFG: Failed to initialize address-to-block look up");
        }

        long imageBase = this.program.getImageBase().getOffset();
        BasicBlockModel basicBlockModel =
            new BasicBlockModel(this.program, true);
        ReferenceManager referenceManager = this.program.getReferenceManager();

        // NOTE: Ghidra does not show returns in any case, which is different
        // from GTIRB.
        //       That is, in GTIRB, a RET would generate an edge to a callsite,
        //       Ghidra does not do this.
        ArrayList<Edge> edgeList = new ArrayList<>();
        ArrayList<byte[]> vertexList = new ArrayList<>();
        try {
            CodeBlockIterator codeBlockIterator =
                basicBlockModel.getCodeBlocks(TaskMonitor.DUMMY);
            while (codeBlockIterator.hasNext()) {
                CodeBlock codeBlock = codeBlockIterator.next();
                long startAddress =
                    codeBlock.getFirstStartAddress().getOffset() - imageBase;
                long srcBlockSize = codeBlock.getMaxAddress().getOffset() -
                                    imageBase - startAddress;
                UUID srcUuid =
                    getBlockAtAddress(module, startAddress, srcBlockSize);
                if (srcUuid == null) {
                    // This shouldn't happen normally:
                    Msg.info(
                        this,
                        "Export CFG: Source address does not map to a code block: 0x" +
                            Long.toHexString(startAddress));
                    continue;
                }
                // ReferenceManager.getFlowReferencesFrom() may be useful too.
                CodeBlockReferenceIterator dstIter =
                    codeBlock.getDestinations(TaskMonitor.DUMMY);
                while (dstIter.hasNext()) {
                    CodeBlockReference dstRef = dstIter.next();
                    long dstAddr =
                        dstRef.getDestinationAddress().getOffset() - imageBase;
                    long dstBlockSize = dstRef.getDestinationBlock()
                                            .getMaxAddress()
                                            .getOffset() -
                                        imageBase - dstAddr;
                    UUID dstUuid =
                        getBlockAtAddress(module, dstAddr, dstBlockSize);
                    if (dstUuid == null)
                        continue;
                    ...
                    if (oldEdges.contains(flow)) {
                        // This Ghidra edge matches one that already existed in
                        // GTIRB
                        continue;
                    }

                    Edge edge =
                        makeEdge(srcUuid, dstUuid, dstRef.getFlowType());
                    edgeList.add(edge);
                }
            }
        } catch (Exception e) {
            throw new ExporterException(
                "Export CFG: Unable to populate new edges: " + e);
        }
