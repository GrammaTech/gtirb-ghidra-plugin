...
public class DumpComments extends GhidraScript {

    private static final String[] COMMENT_TYPES = {"EOL", "Pre", "Post",
                                                   "Plate", "Repeatable"};
        ...
        while (commentAddresses.hasNext()) {
            ...
            for (int i = 0; i < COMMENT_TYPES.length; i++) {
                ...
                if (commentValue != null) {
                    printf(" %s : %s : %s", address.toString(),
                           COMMENT_TYPES[i], commentValue);
                }
            }
        }
        printf("Dump Comments: OK");
    }
}
