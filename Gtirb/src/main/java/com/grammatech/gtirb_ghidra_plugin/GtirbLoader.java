...
public class GtirbLoader extends AbstractLibrarySupportLoader {
    ...
    private boolean addFunction(GtirbFunction function) {
        ...
        // The location of the call to getDisassembler apparently should be
        // close to the call to actually disassemble. Doing it once for the
        // whole load did not work and resulted in many disassembler errors.
        this.disassembler = Disassembler.getDisassembler(
            this.program, true, true, true, this.monitor,
            DisassemblerMessageListener.CONSOLE);
        this.disassembler.disassemble(entryAddress, body, true);
        return true;
    }

    private boolean addSymbol(Symbol symbol, long address,
                              Namespace namespace) {
    ...
    private boolean addSectionBytes(Section section, MessageLog log,
        ...
        for (ByteInterval byteInterval : byteIntervals) {
            ...
            // Why not just mark the whole byte interval, since the section is
            // executable? Here's the answer: If you do that, the disassembler
            // flags bad instructions on
    ...
    protected void load(ByteProvider provider, LoadSpec loadSpec,
        ...
        // been done to make listings useful. for all sections
        ...
        //                        startAddress + dataBlock.getSize() - 1;
        //                        markAsCode(startAddress+this.loadOffset,
        //                        dataBlock.getSize());
        //
        //                        Address entryAddress =
        //                        imageBase.add(startAddress); Address
        //                        lastAddress = imageBase.add(endAddress);
        //                        AddressSet body = new AddressSet(entryAddress,
        //                        lastAddress);
        //                        this.disassembler.disassemble(entryAddress,
        //                        body, true);
        //
        //            		}
        //            	}
